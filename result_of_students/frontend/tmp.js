 ...
 *  - Reads ?institute=NAME&address=ADDR from outer_web.html navigation
 */
...
/* ── Read institute from URL params (set by outer_web.html) ── */
(function applyInstituteFromUrl() {
  const params      = new URLSearchParams(window.location.search);
  const institute   = params.get("institute");
  const address     = params.get("address");

  if (!institute) return; /* opened directly — keep defaults */

  /* Update marksheet school header */
  const msName = document.getElementById("ms-school-name");
  const msAddr = document.getElementById("ms-school-address");
  if (msName) msName.textContent = institute;
  if (msAddr && address) msAddr.textContent = address;

  /* Update page title and hero sub */
  document.title = `${institute} — Student Search`;

  /* Show selected-institute chip in site header */
  const chip     = document.getElementById("headerInstituteChip");
  const chipName = document.getElementById("headerInstituteName");
  if (chip && chipName) {
    chipName.textContent = institute;
